...
const LatestNews = () => {
  ...
  //so it will show this while fetching the data from the api
    ...
    <Box display={'flex'} flexDirection={'column'} gap={'28px'}>
        <Box>
            {latestNews.slice(0, 1).map((news) => (
                <Box  component={Link} to={`/news/${news.id}`} key={news.id} sx={{height:"187px",display:"flex",justifyContent: {xs:"center", md:"space-between"}, gap:{xs:"20px", md:"0"},overflow:"hidden",cursor:"pointer"}}>
        ...
        <Box sx={{display:"flex",flexWrap:"wrap",justifyContent: {xs:"center", md:"space-between"}, gap:{xs:"20px", md:"0"}}}>
            {latestNews.slice(1).map((news) => (
                <Box component={Link} to={`/news/${news.id}`} key={news.id} sx={{width:{xs:"46%" ,md:"22.5%"},height:"187px",overflow:"hidden", cursor:"pointer"}}>
